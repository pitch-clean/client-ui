...
const ITCol = ({ label, title, investments }) => {
  ...
  const invColArr = investments.map((inv, idx) => {
    const {
      offering: { term, sponsor, financials },
      principalInvested,
      distributions,
    } = inv;
    let input;
    ...
    if (label === 'interest') {
      input = calculateInterestPerInvestment(
        distributions,
        principalInvested,
        term,
        financials.fundTarget,
      );
    }
    if (label === 'principal') {
    ...
    return (
      <ITCell input={input} isFirstCol={idx === 0} isLastCol={idx === investments.length - 1} />
    );
  });

  return (
    <div className={`ITCol ${classes.root} flexcol`}>
